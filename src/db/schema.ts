...
export const users = pgTable(
  "users",
  {
    ...
    /** Authorization role. Only SUPER_ADMIN may reach /super-admin. */
    role: text("role").$type<"USER" | "SUPER_ADMIN">().notNull().default("USER"),
    /** Account lifecycle: active | suspended */
    status: text("status").$type<"active" | "suspended">().notNull().default("active"),
    /** Bumped to instantly invalidate all issued session tokens. */
...
export const auditEvents = pgTable(
  "audit_events",
  {
    ...
    action: text("action").notNull(), // account.created | auth.login | run.created | key.saved | admin.* …
    detail: text("detail").notNull().default(""),
    /** Structured metadata (target ids, result, actor role…). Never secrets. */
...
/** Runtime feature flags togglable from the Super Admin portal. */
...
/** Insights generated by the recommendation engine over real aggregates. */
export const adminInsights = pgTable(
  "admin_insights",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    /** Stable rule key — regenerated insights upsert on this key. */
    ruleKey: text("rule_key").notNull(),
    title: text("title").notNull(),
    description: text("description").notNull(),
    /** JSON evidence: metric name, value, period, query reference. */
    evidence: jsonb("evidence").$type<Record<string, unknown>>().notNull().default({}),
    priority: text("priority").$type<"high" | "medium" | "low">().notNull().default("medium"),
    confidence: text("confidence").$type<"high" | "medium" | "low">().notNull().default("medium"),
    affectedSegment: text("affected_segment").notNull().default(""),
    affectedArea: text("affected_area").notNull().default(""),
  ...
  (t) => ({
    ruleKeyIdx: uniqueIndex("admin_insights_rule_key_idx").on(t.ruleKey),
    statusIdx: index("admin_insights_status_idx").on(t.status),
  })
);
